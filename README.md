...
- **Ticket attachment tools touch the local filesystem.** `ndcli.ticket.attachment_upload` and `ndcli.support.attachment_upload` read a local path on the machine running the server (a regular file, at most 25 MiB); `ndcli.ticket.attachment_download` and `ndcli.support.attachment_download` write one, and refuse to replace an existing file unless the caller passes `overwrite: true`. They are the only tools in the set that read or write local files.
